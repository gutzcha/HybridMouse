 function output=SSBoll79(signal,fs,IS,Gamma)
        ...
        W=fix(0.015*fs); %Window length is 2 ms
        nfft=W;
        SP=.004; %Shift percentage is 40% (10ms) %Overlap-Add method works good with this value(.4)
        wnd=hamming(W);
        
        if ~exist('Gamma', 'var')
            Gamma=0.7;%Magnitude Power (1 for magnitude spectral subtraction 2 for power spectrum subtraction)
        end
    ...
    function dataOut=hpfilter(n,fl,fh)
        %fc=5000;% cut off frequency
        fn=125000; %nyquivst frequency = sample frequency/2;
        order = 1.5; %6th order filter, high pass
        [b14, a14]=butter(order,(fl/fn),'high');
        %fvtool(b14,a14);
        xf14=filtfilt(b14,a14,n);
        [b,a] = butter(order,fh/fn);
        %freqz(b,a)
        dataOut = filter(b,a,xf14);
    end
